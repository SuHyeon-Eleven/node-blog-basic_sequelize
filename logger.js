...
const logger = winston.createLogger({
    ...
    // uncaughtException (예측 불가능한 에러)발생시 파일 설정
...
// production 환경이 아닌 개발환경일경우 화면에서 바로 찍게 설정. 로그파일도 저장됨. 
if(process.env.NODE_ENV !== 'production'){
    logger.add(
        new winston.transports.Console({
            format : winston.format.combine(
                winston.format.colorize(), // 로그 레벨별로 색상 적용
